    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<IActionResult> ServiceEngineers(ServiceEngineerViewModel serviceEngineer)
        {
            serviceEngineer.ServiceEngineers =
                HttpContext.Session.GetSession<List<ApplicationUser>>("ServiceEngineers");
            ...
            if (serviceEngineer.Registration.IsEdit)
            {
                ...
                var removeClaimResult = await _userManager.RemoveClaimAsync(user, new Claim(isActiveClaim.ClaimType, isActiveClaim.ClaimValue));

                var addClaimResult = await _userManager.AddClaimAsync(user, new Claim(isActiveClaim.ClaimType, serviceEngineer.Registration.IsActive.ToString()));
